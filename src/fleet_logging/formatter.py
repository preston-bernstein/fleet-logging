"""The canonical JSON log line — home-infra CONVENTIONS.md §18 (fleet
logging contract).
...
1. **stdlib `logging`-based** (algo-corpus's `logging_setup.py` shape):
...
2. **Direct function call** (algo-macro-monitor's `log.py` shape), for a
...
# algo-corpus's logging_setup.py — same reasoning applies here.)
...
class JsonFormatter(logging.Formatter):
    ...
    matching algo-macro-monitor's `_scrub()` — algo-corpus's original did
...
class _StreamHandler(logging.StreamHandler):
    ...
    verbatim from algo-corpus's `_StdoutHandler`, generalized to either
    stream.)
    """
...
def configure_logging(
    ...
    algo-macro-monitor's deliberate stdout/stderr split (machine-readable
...
def new_run_id(prefix: str = "run") -> str:
    ...
    process already minted one (algo-corpus's behavior — lets a shell script
    and the Python subprocess it launches share one id end to end), else
    mints a fresh time-prefixed, randomly-suffixed id (algo-macro-monitor's
    behavior — the random suffix avoids two invocations in the same second
    colliding, which algo-corpus's PID-suffixed version did not fully
...
def log_event(
    ...
    **fields,
    ...
    module configuration required — algo-macro-monitor's shape, for a
    ...
    `sys.stderr` to match algo-macro-monitor's convention.
